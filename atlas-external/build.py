...
# ---- AUTHORITATIVE FINAL ROUTE SWEEP (path-independent; runs after ALL sources merge) ----
# Belt-and-suspenders over curated-loop + route_network + ICR. Two guarantees that must hold
# regardless of which code path emitted an edge (this is what makes the build deterministic-safe
# against F-11 route-set non-determinism):
#   1. Rule A: any Quanta-LR edge whose real distance is <=70 nm is reclassified to Pioneer II.
#   2. HARD flag-and-exclude: drop any route whose endpoint is a documented route-specific
#      HARD-exclude destination (reference/politically-active-flag-and-exclude-zones.md §(a)).
#      These are geopolitically non-viable commercial legs that must never render.
_HARD_ROUTE_EXCLUDE = ("eilat", "israel", "port-sudan", "socotra", "mukalla", "sarfait")
_final_ruleA = 0
_final_excluded = []
_kept = []
for _f in route_features:
    _p = _f.get("properties", {})
    _eps = f"{_p.get('from','')} {_p.get('to','')}".lower()
    if any(_tok in _eps for _tok in _HARD_ROUTE_EXCLUDE):
        _final_excluded.append({"id": _p.get("id"), "from": _p.get("from"), "to": _p.get("to"),
                                "platform": _p.get("platform"), "distance_nm": _p.get("distance_nm")})
        continue
    _d = _p.get("distance_nm") or 0
    if _p.get("platform") == "Quanta-LR" and _d and _d <= PIONEER_MAX_NM:
        _p["platform"] = "Pioneer II"; _final_ruleA += 1
    _kept.append(_f)
route_features = _kept
print(f"Final sweep: Rule A reclassified {_final_ruleA} QLR->Pioneer II (<=70nm); "
      f"dropped {len(_final_excluded)} HARD flag-and-exclude routes")
for _x in _final_excluded:
    print(f"   [excluded] {_x['from']} -> {_x['to']} ({_x['distance_nm']}nm {_x['platform']})")
(HERE / "route_hard_exclude_audit.json").write_text(json.dumps(_final_excluded, indent=2, ensure_ascii=False))
