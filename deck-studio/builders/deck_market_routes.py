...
def _short_label(journey: dict) -> str:
    return f"{journey['from']} → {journey['to']}"
...
def market_route_style_ranges(text: str) -> list[dict]:
    """Amber for each ▸ bullet; white Exo-2 11pt bold for all other characters."""
    runs: list[dict] = []
    i = 0
    white = {**MARKET_ROUTE_FONT, "color": MARKET_ROUTE_WHITE}
    ...
    while i < len(text):
        if text[i] == "▸":
            ...
            while j < len(text) and text[j] != "▸":
                j += 1
            runs.append({"start": i, "end": j, "style": white})
            i = j
    return runs
...
def market_route_replace_ops(
    ...
    *,
    ...
    for i, run in enumerate(market_route_style_ranges(new_text)):
...
def resolve_binding_routes(binding: dict, markets: dict[str, dict]) -> tuple[list[dict], str]:
    ...
    source = f"data-clean/partners/bolt.json markets.{binding['market_id']}.journeys_unlocked"
    return journeys, source
...
def build_market_route_ops(
    ...
    *,
    ...
    for binding in bindings_doc["bindings"]:
        journeys, source = resolve_binding_routes(binding, markets)
...
def validate_market_route_bindings(deck_key: str, *, partner_json_path: Path, golden: dict) -> list[str]:
    ...
    for binding in bindings_doc["bindings"]:
        ...
        try:
            ...
            journeys, _ = resolve_binding_routes(binding, markets)
            text = format_route_list(
                journeys,
                taglines=binding.get("taglines"),
                labels=binding.get("labels"),
