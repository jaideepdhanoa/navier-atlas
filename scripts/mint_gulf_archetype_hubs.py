...
HAND_PATHS: dict[tuple[str, str], list[list[float]]] = {
    ...
    ("qawasim-2", "hilton-corniche"): [
        [55.9480, 25.7915],
        [55.950, 25.7925],
        [55.9525, 25.7940],
    ],
    ("royal-yacht-club", "hilton-corniche"): [
        [55.778509, 25.695935],
        [55.785, 25.715],
        [55.810, 25.735],
        [55.850, 25.755],
    ...
    ("jazirat-al-hamra", "royal-yacht-club"): [
        [55.7960, 25.7085],
        [55.800, 25.710],
        [55.790, 25.702],
        [55.778509, 25.695935],
...
def _land_masks():
    ...
    # Also buffer every hand path so densified hand polylines stay in-corridor by design
    for pts in HAND_PATHS.values():
        if len(pts) >= 2:
            corridors.append(LineString(densify(pts, 0.15)))
    # ~500 m buffer around authored waterways
    water_buf = unary_union([ln.buffer(0.005) for ln in corridors])
    water_prep = prep(water_buf)
...
def _point_is_land_gated(lon: float, lat: float) -> bool:
    ...
    if water_prep.intersects(Point(lon, lat)):
        return False
...
def land_qa(coords: list[list[float]]) -> dict:
    """Corridor-gated land QA (canonical gate for Gulf hubs)."""
    ...
    return {
        ...
        "mask": "uae_wkb_v2+waterway_corridors",
...
def bind_segment(
    ...
    if hand and len(hand) >= 2:
        ...
        if ev.get("qa_pass"):
            ...
            # Still use hand if it's the best visual corridor; craft may not improve canal legs
            coords, method, qa = path, "hand_waterway", ev
...
RAK_HAND_COORDS = {
    ...
    # RAKTA abra-style corniche stations (research approx along RAK corniche)
    "qawasim-1": (55.9435, 25.7885, None),
    "qawasim-2": (55.9480, 25.7915, None),
    "hilton-corniche": (55.9525, 25.7940, None),
    # Jazirat Al Hamra heritage shore
    "jazirat-al-hamra": (55.7960, 25.7085, None),
}
...
def build_rak(receipt: dict) -> dict:
    ...
    labels = {
        ...
        "qawasim-1": "Qawasim Corniche 1",
        "qawasim-2": "Qawasim Corniche 2",
        "hilton-corniche": "Hilton Garden Inn Corniche Station",
        "jazirat-al-hamra": "Jazirat Al Hamra waterfront",
    }
    for key, (lng, lat, bp) in RAK_HAND_COORDS.items():
        stops[key] = resolve_stop(key, labels[key], lng, lat, bp)
        if key in ("al-marjan", "mina-al-arab", "jazirat-al-hamra"):
    ...
    lines = []
    # MRJ-1 Resort Shuttle: Al Marjan ↔ Royal Yacht Club ~4 nm
    lines.append(
        make_line(
            "MRJ-1",
            "Resort Shuttle",
            "#e0cb8f",
            ["al-marjan", "royal-yacht-club"],
            [seg("al-marjan", "royal-yacht-club", 24)],
            flagship=True,
        )
    )
    # MNA-1 City Line: Mina Al Arab ↔ Corniche
    lines.append(
        make_line(
            "MNA-1",
            "City Line",
            "#7dd3c0",
            ["mina-al-arab", "qawasim-1", "qawasim-2", "hilton-corniche"],
            [
                seg("mina-al-arab", "qawasim-1", 37),
                seg("qawasim-1", "qawasim-2", 8),
                seg("qawasim-2", "hilton-corniche", 8),
            ],
        )
    )
    # SPN-1 Coastal Spine: Al Marjan ↔ Corniche ~17 nm
    lines.append(
        make_line(
            "SPN-1",
            "Coastal Spine",
            "#9bb7ff",
            ["al-marjan", "royal-yacht-club", "hilton-corniche"],
            [
                seg("al-marjan", "royal-yacht-club", 24),
                seg(
                    "royal-yacht-club",
                    "hilton-corniche",
                    57,
                    "≈57 min day · ≈65 min after dark (20 kn night planning basis)",
                ),
            ],
        )
    )
    # HER-1 Heritage: Creek/Corniche ↔ Jazirat Al Hamra
    lines.append(
        make_line(
            "HER-1",
            "Heritage Line",
            "#e8a87c",
            ["qawasim-1", "jazirat-al-hamra", "royal-yacht-club"],
            [
                seg("qawasim-1", "jazirat-al-hamra", 48),
                seg("jazirat-al-hamra", "royal-yacht-club", 20),
            ],
        )
    )
    ...
    return base_hub(
        ...
        contact="jaideep@navierboat.com",
        notes="RAK geometry is hand-crafted (sealed corridor set contaminated — no route_ids bound). All paths land-QA gated.",
    )
