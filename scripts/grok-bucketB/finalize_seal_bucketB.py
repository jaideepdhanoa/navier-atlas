#!/usr/bin/env python3
"""Update SEAL.json after Bucket B apply (bytes-truth + fresh timestamp)."""
...
def main():
    ...
    ap.add_argument("--seal", default="#79an")
    args = ap.parse_args()
    ...
    seal["gates"]["land_crossing"] = (
        f"PASS grok-bucketB apply — LB-242 allowlist + LB-224 postflight ({args.seal})"
    )
    ...
    print(json.dumps({"SEAL finalized": args.seal, "routes": routes_n}, indent=2))

    reseal = dc.parent.parent / "atlas-external" / "reseal_from_disk.py"
    if reseal.exists():
        subprocess.run([sys.executable, str(reseal), "--dry-run", "--src", str(dc)], check=False)
