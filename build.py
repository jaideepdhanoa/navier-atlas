...
<div id="toggles">
  ...
  <button class="toggle steel on" id="t-pois" title="Boarding points: marinas, ferry terminals, hotel jetties, water-taxi stops">Boarding pts</button>
</div>

<div id="legend">
  <div class="head">Nodes <span class="help" data-tip="Cities = whole markets. Areas = sub-districts/islands inside a city. Boarding points = real coastal pickup spots (marina, jetty, ferry terminal, water-taxi stop).">?</span></div>
  ...
  <div class="row" style="margin-top:8px"><span class="dot bp marina"></span>Marina / yacht basin</div>
  <div class="row"><span class="dot bp ferry"></span>Ferry / cruise / water-bus</div>
  <div class="row"><span class="dot bp yacht"></span>Yacht club</div>
  <div class="row"><span class="dot bp hotel"></span>Hotel / resort / beach-club jetty</div>
  <div class="row"><span class="dot bp seaplane"></span>Seaplane base</div>
  <div class="row"><span class="dot bp watertaxi"></span>Water-taxi / abra / public pier</div>
  <div class="row"><span class="dot bp harbour"></span>Working harbour</div>
  <div class="row" style="margin-top:8px"><span class="line"></span>Pioneer II route <span class="muted">— ≤70 nm electric</span></div>
  <div class="row"><span class="line qlr"></span>Quanta-LR route <span class="muted">— ≤2,000 nm hybrid</span></div>
</div>
...
map.on('load', () => {
  ...
  const BP_COLOR = ['match', ['coalesce', ['get','bp_type'], 'default'],
    'marina','#60a5fa',
  ...
  const BP_RADIUS = ['match', ['coalesce', ['get','bp_type'], 'default'],
    'marina',5, 'working_harbour',4.5, 'ferry_terminal',5, 'cruise_terminal',5,
    'water_bus_terminal',4.5, 'yacht_club',4.5, 'hotel_jetty',4, 'resort_jetty',4,
    'beach_club_jetty',4, 'seaplane_base',4.5, 'water_taxi_stop',3.5,
    'abra_station',3.5, 'public_pier',3.5, 'floating_pontoon',3.5, 'floating_helipad',4,
    3.5
  ];
  map.addLayer({ id:'poi-halo', type:'circle', source:'pois', filter:['!',['has','point_count']], minzoom:10,
    paint:{ 'circle-color':BP_COLOR, 'circle-opacity':0.22, 'circle-radius':12, 'circle-blur':0.6 }});
  map.addLayer({ id:'poi-points', type:'circle', source:'pois', filter:['!',['has','point_count']], minzoom:9,
    paint:{ 'circle-color':BP_COLOR, 'circle-radius':BP_RADIUS, 'circle-stroke-width':1.2, 'circle-stroke-color':'#0a0e14' }});
  ...
  for (const layer of ['city-points','city-halo','city-clusters','city-cluster-halo','locale-points','locale-halo','locale-clusters','poi-points','poi-halo','poi-clusters']) {
