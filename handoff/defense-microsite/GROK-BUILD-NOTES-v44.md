...
- Placement: after `def-flight` (04 · PROOF IN FLIGHT), not after Control — vessel first, then autonomy.
- Page-wide: `.section-inner` / `.media-inner` / `.shell-stage` share `--stage` (1440) so kickers line up with content.
