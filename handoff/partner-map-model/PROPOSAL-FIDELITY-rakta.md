# Proposal fidelity — rakta

**Verdict:** PASS
**Checked:** 2026-06-29T15:14:15Z

## Summary

- Items audited: 12
- KEEP: 12
- DROP: 0
- DEFER: 0
- TRIM/REWRITE: 0
- BP-binding errors: 0

## Trim list

| Surface | Phase | Corridor | Route | Rec | Flags |
|---------|-------|----------|-------|-----|-------|
| journey | — | RAK Harbour / Corniche → Al Marjan and Mina Al Ara | `—` | **KEEP** | — |
| journey | — | Dubai Harbour → Wynn Al Marjan Island | `—` | **KEEP** | — |
| journey | — | RAK / UAE → Musandam, Muscat, Doha, Bahrain | `—` | **KEEP** | — |
| featured | 1 | Al Marjan Island public arrival marina → Rixos Bab | `rn-f0d6b20a125c` | **KEEP** | — |
| featured | 1 | Ras Al Khaimah Harbour → RAK Corniche public pier | `rn-2a5c2fe11732` | **KEEP** | — |
| featured | 1 | Marjan Island Resort & Spa beach jetty → Hampton b | `rn-67316de45b8f` | **KEEP** | — |
| featured | 2 | Dubai Harbour Marina → Wynn Al Marjan Island arriv | `gcn-9e515da38a-bolt` | **KEEP** | — |
| featured | 2 | Ras Al Khaimah → Abu Dhabi | `—` | **KEEP** | — |
| featured | 2 | Ras Al Khaimah → Sharjah | `—` | **KEEP** | — |
| featured | 3 | Ras Al Khaimah → Muscat | `—` | **KEEP** | — |
| featured | 3 | Ras Al Khaimah → Doha | `—` | **KEEP** | — |
| featured | 3 | Ras Al Khaimah → Manama | `—` | **KEEP** | — |
